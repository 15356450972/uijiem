...
def _dola_capture_one(body: DolaAccountGrab, index: int, total: int) -> dict:
    ...
    result = dl.grab_account(
        ...
        proxy=body.proxy,
        send_hi=body.send_hi,
        hi_text=body.hi_text,
...
def _dola_video_quota_cost(duration: int) -> int:
    normalized = max(1, int(duration or 5))
    return max(1, (normalized + 4) // 5)


def _dola_pick_account(account_id: int = 0, quota_cost: int = 0) -> dict | None:
    if account_id:
    ...
    if quota_cost > 0:
        account = DolaAccountDB.reserve_best_daily_video_quota(quota_cost)
        if account:
            return account
    ...
    if active_accounts:
        active_accounts.sort(key=lambda a: (-int(a.get("daily_video_remaining") or 0), float(a.get("updated_at") or 0), int(a.get("id") or 0)))
...
def dola_update_config(body: DolaConfigUpdate):
    """保存 Dola 通道配置。"""
    try:
        if body.proxy is not None or body.env_file is not None or body.profile_dir is not None:
            dl.save_config(proxy=body.proxy, env_file=body.env_file, profile_dir=body.profile_dir)
        return {"data": dl.account_status()}
    except dl.DolaError as e:
        raise HTTPException(status_code=e.status_code or 500, detail=str(e))
    except Exception as e:
...
def dola_list_accounts():
    """Dola 采集账号库。"""
    return {"data": DolaAccountDB.list_all()}
...
def dola_import_account(body: DolaAccountImport):
    """把已有 Dola 登录态文件加入账号库。"""
    try:
        if body.env_file or body.profile_dir:
            dl.save_config(env_file=body.env_file or None, profile_dir=body.profile_dir or None)
        account = _dola_save_current_account(body.name, body.note)
        return {"data": account}
    except dl.DolaError as e:
        raise HTTPException(status_code=e.status_code or 500, detail=str(e))
    except Exception as e:
...
def dola_open_account_browser(account_id: int, body: DolaAccountOpenBrowser | None = None):
    ...
    try:
        ...
        if not conversation_id and payload.task_id:
            try:
                task_detail = dl.get_task_status(payload.task_id)
                conversation_id = str(task_detail.get("conversation_id") or "").strip()
            except dl.DolaError:
                conversation_id = ""
        target_url = f"https://www.dola.com/chat/{conversation_id}" if conversation_id else ""
...
def dola_create_task(body: DolaTaskCreate):
    """通过 Dola 提交视频生成任务（后台线程执行）。"""
    try:
        quota_cost = _dola_video_quota_cost(body.duration)
        account = _dola_pick_account(body.account_id, quota_cost=quota_cost)
        ...
        if account:
            ...
            result["daily_video_quota"] = account.get("daily_video_quota", 6)
            result["daily_video_used"] = account.get("daily_video_used", 0)
            result["daily_video_remaining"] = account.get("daily_video_remaining", 0)
        task_id = result.get("task_id", "")
        if task_id:
            TaskDB.add(
                task_id=task_id,
                ...
                prompt=body.prompt,
                model=f"dola:{body.model}:{int(body.duration or 5)}s",
...
def dola_task_status(task_id: str):
    ...
    try:
        ...
        return {"data": {
            ...
            "queue_position": None,
            "fail_reason": detail.get("error", ""),
            "collectable": bool(detail.get("conversation_id")) and status != "completed",
...
def dola_collect_task(body: DolaTaskCollect):
    ...
    try:
        account = _dola_pick_account(body.account_id)
...
def dola_collect_task_by_id(task_id: str, body: DolaTaskCollect | None = None):
    ...
    try:
        ...
        account = _dola_pick_account(payload.account_id)
